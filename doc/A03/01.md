...
### 前準備 spring-boot 2.4 用の定義ファイルを作る

2021/4 時点で、公式に Release されている Wicket-Spring-Boot-Starter （WicketとSpringをつなぐプロジェクト）は、Spring, Wicketのバージョンが低いままになっている。

これを解決するための方法として、独自にバージョン番号の設定済みの定義を用意する。

1. まず、作成した wsbp プロジェクトの直下に、`pom-parent` フォルダを作成する。
2. 次に、 `pom-parent` フォルダの中に、[Spring-boot-24.xml](https://raw.githubusercontent.com/gishi-yama/wicket_spring-boot_practice/master/wsbp/pom-parent/spring-boot-24.xml) ファイルをダウンロードしてコピーする。

以降は、pom.xmlファイルを編集していく。

### 1 pom.xml の `<parent>...</parent>` を書き換える
