...
- IntelliJ IDEA
    ...
    - import Maven projects automatically(Mavenプロジェクトを自動的にインポートする) にチェックして続行
...
- 自分のPCにIntelliJ IDEAをインストールしている学生は、日本語化やWicketForgeプラグインをいれるとはかどる。
    ...
    - [WicketFrogeプラグイン](https://plugins.jetbrains.com/plugin/1545-wicketforge)
        - 設定(Setting) > プラグイン(plugins) > Browser Repositories で WicketForge を検索し、インストール  
