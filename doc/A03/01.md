...
- Project を `maven` にする
- Language を `Java` にする
- Spring Boot を `2.7` の最新版(SNAPSHOTは除く) なければ 2.X の最新版にする
- Artifact を `wsbp` にする
- Java を `17` にする
...
### 前準備 spring-boot 3.7 用の定義ファイルを作る
...
2. 次に、 `pom-parent` フォルダの中に、[Spring-boot-27.xml](https://raw.githubusercontent.com/gishi-yama/wicket_spring-boot_practice/master/wsbp/pom-parent/spring-boot-27.xml) ファイルをダウンロードしてコピーする。
